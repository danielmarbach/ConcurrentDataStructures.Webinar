namespace NServiceBus.AzureServiceBus
{
    ...
    class MultiProducerConcurrentCompletion<TItem>
    {
        public MultiProducerConcurrentCompletion(int batchSize, TimeSpan pushInterval, int maxConcurrency, int numberOfSlots)
        {
            ...
            channels = new Channel<TItem>[numberOfSlots];
            itemListBuffer = new ConcurrentQueue<List<TItem>>();

            for (var i = 0; i < numberOfSlots; i++)
            {
                channels[i] = Channel.CreateUnbounded<TItem>(new UnboundedChannelOptions
                {
                    SingleReader = true,
                    SingleWriter = false,
        ...
        public void Push(TItem item, int slotNumber)
        {
            ...
            if (!channels[slotNumber].Writer.TryWrite(item))
            {
                return;
            }
        ...
        void PushInBatchesUpToConcurrencyPerQueueForAGivenSlot(Channel<TItem> queue, int currentSlotNumber, List<Task> tasks)
        {
            int numberOfItems;
            var concurrency = 1;
            do
